...
function Build-WebKitDockerImage {
  ...
  $cmd = 'docker build -t webkitdev/{0}:{1} {2} -f {3} --build-arg HTTPS_PROXY={5} --build-arg HTTP_PROXY={6} -m 2GB {4}' -f $image,$tag,$buildArgs,$file,$path,$env:HTTPS_PROXY,$env:HTTP_PROXY;
